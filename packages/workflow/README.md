...
不发布到 npm。它是私有包，会被 `mcp-server` 和 `agent-cli` 的构建产物打包进去。

## 为什么存在

`mcp-server` 和 `agent-cli` 都需要执行相同业务流程：扫描项目、采集 raw、读取 raw、保存 summary。如果这些逻辑分别写在两个入口里，会造成重复维护。
...
```text
mcp-server -> workflow -> core -> shared
