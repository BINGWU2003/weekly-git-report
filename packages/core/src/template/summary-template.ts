...
import { PeriodSchema } from "@weekly-git-report/shared";
...
export const DEFAULT_SUMMARY_TEMPLATE = `# 周报总结生成规则
...
export function renderSummaryTemplate(content: string, period: Period): string {
...
export async function initializeSummaryTemplate(
  ...
  const templateFile = options.templateFile ?? getSummaryTemplateFilePath();
  await mkdir(path.dirname(templateFile), { recursive: true });

  let created = false;
  try {
    await writeFile(templateFile, DEFAULT_SUMMARY_TEMPLATE, { encoding: "utf8", flag: "wx" });
    created = true;
  } catch (error) {
    if (!isNodeError(error) || error.code !== "EEXIST") throw error;
  }

  return buildSummaryTemplateResult(templateFile, created, options.period);
}
...
export async function saveSummaryTemplate(
  ...
  return buildSummaryTemplateResult(templateFile, false, options.period);
}
...
export async function resetSummaryTemplate(
  ...
  await writeTextAtomic(templateFile, DEFAULT_SUMMARY_TEMPLATE);
  return buildSummaryTemplateResult(templateFile, false, options.period);
}
...
async function buildSummaryTemplateResult(
  ...
  const defaultRevision = sha256(DEFAULT_SUMMARY_TEMPLATE);
  ...
  return {
    ...
    template: {
      content,
      renderedContent: period ? renderSummaryTemplate(content, period) : null,
      path: templateFile,
